        ...
        let db = {
			...
			equipped_weapon: localStorage.getItem('roach_equipped_weapon') || 'slipper'
        };
...
function isDuplicate(prize) {
    if (prize.category === 'border') {
        return db.unlocked_borders && db.unlocked_borders.includes(prize.id);
    }
    if (prize.category === 'title') {
        return db.unlocked_shop_titles && db.unlocked_shop_titles.includes(prize.id);
    }
    return false;
}

function applyPrize(prize, duplicate) {
    if (duplicate) {
        if (prize.duplicateCoins) {
            db.coins += prize.duplicateCoins;
        }
        return;
    }
    if (prize.category === 'border') {
        if (!db.unlocked_borders) db.unlocked_borders = ['none'];
        db.unlocked_borders.push(prize.id);
    } else if (prize.category === 'title') {
        if (!db.unlocked_shop_titles) db.unlocked_shop_titles = [];
        db.unlocked_shop_titles.push(prize.id);
    } else if (prize.category === 'coins') {
        db.coins += prize.rewardCoins;
    } else if (prize.category === 'exp') {
        db.exp += prize.rewardExp;
        if (typeof checkLevelUp === 'function') checkLevelUp();
    }
}

function gachaResultHTML(prize, duplicate) {
    if (duplicate) {
        return {
            rarityText: '✨【命運交織 · 金幣轉化】✨',
            rarityClass: 'text-xs font-black tracking-widest text-cyan-400',
            glowClass: 'absolute inset-0 bg-cyan-500/20 blur-xl rounded-full',
            html: `
                <div class="bg-zinc-900 border border-cyan-800 p-4 rounded-xl text-center">
                    <span class="text-xl">💰</span>
                    <div class="text-cyan-400 font-extrabold text-sm mt-1">【重複補償】${prize.duplicateCoins.toLocaleString()} 金幣</div>
    ...
    if (prize.category === 'border') {
        return {
            rarityText: prize.rarityLabel,
            rarityClass: prize.rarityClass,
            glowClass: prize.glowClass,
            html: `
                <div class="${prize.displayBgClass}">
                    <span class="text-white font-black text-sm text-center">${prize.name}</span>
                    <span class="text-[10px] text-zinc-400 mt-2">${prize.displayDesc}</span>
                </div>
            `
        };
    }
    if (prize.category === 'title') {
        return {
            ...
            html: `
                <div class="${prize.displayBgClass}">
                    <div class="${prize.displayClassName} text-sm text-center">${prize.name}</div>
                    <span class="text-[10px] text-zinc-400 mt-2">${prize.displayDesc}</span>
                </div>
            `
        };
    ...
    return {
        ...
        html: `
            <div class="bg-zinc-900 border border-zinc-800 py-3 px-6 rounded-xl text-center">
                <span class="text-2xl">${prize.displayIcon || '🎁'}</span>
                <div class="text-zinc-300 font-bold text-xs mt-1">${prize.displayDesc}</div>
            </div>
        `
    };
}
...
function renderGachaPreview() {
    ...
    if (!stage) return;
    const heroPrizes = GACHA_CONFIG.prizes.filter(p => p.category === 'border' || p.category === 'title');
    if (heroPrizes.length === 0) return;
    stage.innerHTML = `
        <div class="flex flex-col items-center">
            <span class="text-[10px] text-purple-400 mb-3 font-black tracking-widest animate-pulse">🎯 全部可獲得大獎 ✨</span>
            <div class="flex flex-wrap justify-center gap-3 w-full max-w-sm">
                ${heroPrizes.map(prize => `
                    <div class="${prize.displayBgClass.replace('w-56', 'w-[160px]')} py-3 px-2">
                        <div class="${prize.displayClassName} text-[10px] tracking-widest mb-2 text-center leading-tight">${prize.name}</div>
                        <div class="w-10 h-10 rounded-full bg-zinc-800 flex items-center justify-center mb-1.5 border border-zinc-600 mx-auto"><i class="fa-solid fa-user text-lg text-zinc-400"></i></div>
                        <div class="text-white font-bold text-[10px] opacity-80">${prize.displayDesc}</div>
                    </div>
                `).join('')}
            </div>
        </div>
...
function playGacha() {
    if (db.coins < GACHA_CONFIG.cost) {
        alert(`❌ 金幣不足 ${GACHA_CONFIG.cost}！大魔王在笑你窮，快去刷怪！`);
        return;
    }

    db.coins -= GACHA_CONFIG.cost;
    saveDB();
    ...
    rewardState.classList.add('scale-0');
    ...
    const prize = weightedPick(GACHA_CONFIG.prizes);
    const duplicate = isDuplicate(prize);
    applyPrize(prize, duplicate);
    saveDB();

    setTimeout(() => {
        shakingState.classList.add('hidden');

        const display = gachaResultHTML(prize, duplicate);
        rewardRarity.innerText = display.rarityText;
        rewardRarity.className = display.rarityClass;
        glowBg.className = display.glowClass;
        rewardBox.innerHTML = display.html;
...
function spawnBossCockroach() {
    ...
    bossAttackInterval = setInterval(() => {
        ...
        window.addEventListener('resize', () => {
            ...
            ['target-cake', 'target-cookie', 'target-chicken', 'target-sponge', 'target-boss-food'].forEach(id => {
        ...
        function saveDB() {
			...
			localStorage.setItem('roach_equipped_weapon', db.equipped_weapon);
            updateLobbyUI();
        }
        ...
        function registerKillCombo(now, type) {
            ...
            if(type === 'water') { baseExp = 20; baseCoin = 4; }
            ...
            const finalScore = (type==='normal'?10:type==='tank'?20:type==='golden'?50:type==='water'?20:30) + (gameState.combo * 2);
            ...
            if (pvpState.active) {
                ...
                else if (type === 'water') addPvpCoins = 20;
        ...
        // ==========================================
        function getWeaponSVG(key, sizeClass) {
            const c = WEAPON_CONFIG[key];
            return c.svg ? c.svg.replace('<svg', `<svg class="${sizeClass||'w-8 h-8'}"`) : `<i class="fa-solid ${c.icon} ${c.color} ${sizeClass||'text-2xl'}"></i>`;
        }
    ...
    const ew = db.equipped_weapon || 'slipper';
    const ewConf = WEAPON_CONFIG[ew];
    const ewLvl = db.w_levels[ew] || 1;
    const ewDisplay = document.getElementById('equipped-weapon-display');
    if (ewDisplay) {
        const count = 6 + (ewLvl - 1) * 2;
        const fireY = -(20 + ewLvl * 2);
        const dur = Math.max(0.35, 0.7 - (ewLvl - 1) * 0.035);
        const maxSize = 3 + ewLvl * 0.3;
        const colors = ['#ef4444','#f97316','#fbbf24','#fef08a','#dc2626','#ea580c'];
        const parts = [];
        for (let i = 0; i < count; i++) {
            const side = Math.random();
            let l, r, t, b;
            if (side < 0.33) { l = Math.random()*16+'px'; b = (Math.random()*6-3)+'px'; }
            else if (side < 0.66) { r = Math.random()*16+'px'; b = (Math.random()*6-3)+'px'; }
            else { t = (Math.random()*6-3)+'px'; l = Math.random()*16+'px'; }
            parts.push({
                bg: colors[Math.floor(Math.random()*colors.length)],
                l, r, t, b,
                w: (3+Math.random()*maxSize)+'px', h: (3+Math.random()*maxSize)+'px',
                d: (Math.random()*0.6)+'s'
            });
        }
        ewDisplay.innerHTML = `
            <div class="relative inline-flex items-center" style="--fire-y:${fireY}px;--fire-dur:${dur}s">
                ${getWeaponSVG(ew, 'w-5 h-5')}
                ${parts.map(p => `<div class="fire-particle" style="background:${p.bg};${p.l?`left:${p.l};`:''}${p.r?`right:${p.r};`:''}${p.t?`top:${p.t};`:''}bottom:${p.b};width:${p.w};height:${p.h};animation-delay:${p.d}"></div>`).join('')}
            </div>
            <span class="text-[10px] font-bold text-white">Lv.${ewLvl}</span>
        `;
    }
    ...
    if (titleEl) {
        ...
        const equippedGachaTitle = GACHA_PRIZES[db.equipped_shop_title];
        if (equippedGachaTitle) {
            titleEl.innerHTML = `${equippedGachaTitle.name} <i class="fa-solid fa-pen-to-square text-[10px] text-zinc-400 opacity-60"></i>`;
            titleEl.className = `text-sm font-bold drop-shadow-sm mb-1 cursor-pointer hover:scale-105 transition-all inline-flex items-center gap-1 ${equippedGachaTitle.className}`;
    ...
    document.getElementById('dex-progress').textContent = `${unlockedDex}/${Object.keys(ROACH_TYPES).length}`;
    ...
    if (playerCard) {
        ...
        const gachaBorder = GACHA_PRIZES[currentBorder];
        if (gachaBorder) {
            playerCard.className += ` ${gachaBorder.className}`;
        ...
        function openSceneSelect(mode) {
            ...
            ['living', 'dining', 'bathroom'].forEach(scene => {
        ...
        function selectScene(sceneId) {
            const costs = { living: 800, dining: 2500, bathroom: 12000 };
            if (sceneId !== 'kitchen' && sceneId !== 'boss' && !db.scenes.includes(sceneId)) {
            ...
            if(sceneId === 'boss') {
                gameState.mode = 'boss'; isBossMode = true;
    ...
    clearInterval(bossMoveInterval);
    clearInterval(bossAttackInterval);
    const bossEl = document.getElementById('boss-monster');
...
function updateOnlinePlayersUI() {
    ...
    if (countEl) countEl.innerText = count;
            ...
            Object.keys(WEAPON_CONFIG).forEach(w => {
                ...
                list.innerHTML += `
                    <div class="bg-zinc-800/80 rounded-xl p-3 border ${unlocked ? 'border-zinc-600' : 'border-zinc-800'} flex items-center gap-3">
                        <div class="w-12 h-12 rounded-lg bg-zinc-900 flex items-center justify-center border border-zinc-700 flex-shrink-0" style="${unlocked && lvl>1 ? `box-shadow: 0 0 ${Math.min(lvl*3, 30)}px ${conf.glow}` : ''}">
                            ${unlocked ? getWeaponSVG(w, 'w-7 h-7') : `<div style="filter:grayscale(1) opacity(0.4)">${getWeaponSVG(w, 'w-7 h-7')}</div>`}
                        </div>
                        <div class="flex-1 min-w-0">
        ...
        class Cockroach {
            constructor(x, y) {
                ...
                if (gameState.mode === 'battle') {
                    const allowed = SCENE_CONFIG[gameState.scene].roachTypes;
                    if (gameState.isNight && r < 0.25 && allowed.includes('ninja')) this.type = 'ninja';
                    else if (r < 0.1 && allowed.includes('golden')) this.type = 'golden';
                    else if (r > 0.8 && allowed.includes('tank')) this.type = 'tank';
                    else if (allowed.includes('water') && r > 0.5) this.type = 'water';
                }
                ...
                else if (this.type === 'water') { this.hp = 2 + Math.floor(gameState.score/200); this.speed *= 1.2; }
            ...
            createDOM() {
                ...
                if(this.type==='water'){ bFill='url(#water-grad)'; hFill='#22d3ee'; strk='#164e63'; }
        ...
        function createSplatDebris(x,y,scale,angle,type) {
            ...
            if(type==='water'){jc='#22d3ee';dc='#a5f3fc';sc='#155e75';}
        ...
        function createFireEffect(x,y) {
            ...
            for(let i=0;i<14;i++){
                ...
                c.setAttribute('cx',x+Math.cos(a)*d); c.setAttribute('cy',y+Math.sin(a)*d);
                c.setAttribute('r',4+Math.random()*8); c.setAttribute('fill',colors[Math.floor(Math.random()*colors.length)]);
                c.setAttribute('opacity','0.7'); c.style.filter='blur(3px)';
                containers.waves.appendChild(c);
                let px=x+Math.cos(a)*d, py=y+Math.sin(a)*d;
                let vx=(Math.random()-0.5)*3, vy=-(2+Math.random()*4);
                let life=0, max=20;
                function step(){
                    px+=vx; py+=vy; vy-=0.05; life++;
                    c.setAttribute('cx',px); c.setAttribute('cy',py);
                    c.setAttribute('r',Math.max(0,(4+Math.random()*8)*(1-life/max)));
                    c.setAttribute('opacity',0.7*(1-life/max));
                    if(life<max) requestAnimationFrame(step); else c.remove();
                }
                step();
            }
        }
        ...
        function triggerAttack(x, y, isSwipe=false) {
            ...
            if (gameState.weapon === 'pesticide_bomb') {
                playSound.boom(); createSpray(x,y); createWaveRing(x,y); createFireEffect(x,y);
                let hitAny=false;
            ...
            if(hitAny) createFireEffect(x, y);
            updateBattleUI();
        ...
        function showWeaponVisual(x,y) {
            ...
            const wc = WEAPON_CONFIG[gameState.weapon];
            if (!wc || !wc.svg) return;
            let a = -30;
            if (gameState.weapon==='newspaper') a = -45;
            else if (gameState.weapon==='pesticide_bomb') a = 15;
            ws.innerHTML = wc.svg.replace('<svg', '<svg class="w-full h-full"');
            ws.style.left=`${x-48}px`; ws.style.top=`${y-48}px`; ws.style.transform=`scale(1.2) rotate(${a}deg)`; ws.style.opacity='1';
        ...
        function buildInGameWeaponBar() {
            ...
            db.weapons.forEach(w => {
                const conf = WEAPON_CONFIG[w]; const lvl = db.w_levels[w] || 1;
                c.innerHTML += `
                    <button data-w="${w}" onclick="gameState.weapon='${w}'; db.equipped_weapon='${w}'; saveDB(); updateBattleUI();" class="weapon-btn flex-1 py-1.5 px-1 rounded-xl border border-zinc-700 bg-zinc-800 font-bold text-xs flex flex-col items-center transition select-none relative overflow-hidden">
                        <span${lvl > 1 ? ` style="filter:drop-shadow(0 0 ${Math.min(lvl*2, 20)}px ${conf.glow});display:inline-flex"` : ' style="display:inline-flex"'}>${getWeaponSVG(w, 'w-5 h-5 sm:w-6 sm:h-6')}</span>
        ...
        function startGameSession() {
            ...
            Object.keys(SCENE_CONFIG).forEach(s => {
